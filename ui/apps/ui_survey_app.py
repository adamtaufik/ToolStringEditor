...
class InputWidget(QWidget):
    def __init__(self, callback, parent=None):
        ...
        # Define file type configurations for only top and bottom
        self.file_types = {
            "top": {
        ...
        manual_input_group.setStyleSheet(GROUPBOX_STYLE)
        manual_input_group.setSizePolicy(
            QSizePolicy.Policy.Expanding,
            QSizePolicy.Policy.Maximum
        )
        manual_layout = QGridLayout(manual_input_group)

        # Date input
        date_label = QLabel("Survey Date:")
        ...
        self.location_edit.setPlaceholderText("e.g., Field Name")

        # Well number input
        well_label = QLabel("Well No.:")
        self.well_edit = QLineEdit()
        self.well_edit.setPlaceholderText("e.g., Well-001")

        # THF (BDF) input
        bdf_label = QLabel("THF (ft BDF):")
        self.bdf_edit = QLineEdit()
        # Regex for positive floats (allow decimals)
        float_validator = QRegularExpressionValidator(QRegularExpression(r'^\d*\.?\d{0,2}$'))
        self.bdf_edit.setValidator(float_validator)
        self.bdf_edit.editingFinished.connect(lambda: self.round_lineedit(self.bdf_edit))

        # DFE (Sea Level) input
        sea_level_label = QLabel("DFE (ft AMSL):")
        self.sea_level_edit = QLineEdit()
        ...
        manual_layout.addWidget(time_label, 0, 2)
        manual_layout.addWidget(self.time_edit, 0, 3)

        manual_layout.addWidget(location_label, 1, 0)
        manual_layout.addWidget(self.location_edit, 1, 1)
        manual_layout.addWidget(well_label, 1, 2)
        manual_layout.addWidget(self.well_edit, 1, 3)

        manual_layout.addWidget(bdf_label, 2, 0)
        manual_layout.addWidget(self.bdf_edit, 2, 1)
        manual_layout.addWidget(sea_level_label, 2, 2)
        manual_layout.addWidget(self.sea_level_edit, 2, 3)

        # Station data input
        station_group = QGroupBox("Station Data")
        station_group.setStyleSheet(GROUPBOX_STYLE)
        ...
        # AHD/TVD mapping
        tvd_group = QGroupBox("AHD/TVD Mapping")
        tvd_group.setStyleSheet(GROUPBOX_STYLE)
        tvd_layout = QVBoxLayout(tvd_group)
        ...
        middle_container.setSizePolicy(
            QSizePolicy.Policy.Expanding,
            QSizePolicy.Policy.Maximum
        )
    ...
    def animate_startup(self):
        ...
        for group_box in self.findChildren(QGroupBox):
            ...
            delay += 100  # stagger each box slightly
    ...
    def parse_time_string(self, time_str):
        ...
        # Check for common incorrect delimiters and replace them
        # Replace semicolons, dots, commas, etc. with colons
        # Handle multiple incorrect delimiters in one string
        time_str = re.sub(r'[;.,]', ':', time_str)

        # Handle cases where there might be multiple colons or incorrect spacing
        # Split by colon and clean each part
        parts = time_str.split(':')

        # Filter out empty strings
        parts = [part for part in parts if part]

        # If we don't have exactly 3 parts, try to parse differently
        if len(parts) == 1:
            # Check if it's a continuous string of numbers
            if len(parts[0]) >= 6:
        ...
        if len(parts) >= 3:
            ...
            if hours.isdigit() and minutes.isdigit() and seconds.isdigit():
                ...
                if 0 <= hours_int <= 23 and 0 <= minutes_int <= 59 and 0 <= seconds_int <= 59:
                    return QTime(hours_int, minutes_int, seconds_int)
...
class SurveyApp(QWidget):
    ...
    def init_ui(self):
        ...
        self.title_bar = CustomTitleBar(self, lambda: self.sidebar.toggle_visibility(), "Survey Interpretor")
        ...
        # Change to DualDragDropWidget (for top and bottom files only)
        self.drag_drop_widget = InputWidget(self.process_all_files, self)
        ...
        self.table_widget.verticalHeader().setVisible(False)  # <-- add this
        ...
        self.copy_button.setIcon(QIcon(get_icon_path('copy')))
        ...
        self.process_data_button.setIconSize(QSize(32, 32))
    ...
    def process_all_files(self, top_file_path, bottom_file_path, survey_info):
        """Process files with manual data input from DualDragDropWidget"""
        try:
            self.cleanup_memory()
            self.top_file_path = top_file_path
            self.bottom_file_path = bottom_file_path

            # Set manual data from survey_info dictionary
            self.date = survey_info['date']
            self.start_time = survey_info['start_time']
            self.location = survey_info['location']
            self.well = survey_info['well']
            self.bdf = survey_info['bdf']
            ...
            has_top_data = self.top_data is not None and len(self.top_data) > 0
            has_bottom_data = self.bottom_data is not None and len(self.bottom_data) > 0
            ...
            if has_top_data and has_bottom_data and has_station_timings and has_tvd_data:
                ...
                # Update labels with manual data
                ...
                if not has_tvd_data:
                    missing_items.append("AHD/TVD Data")
    ...
    def show_stacked_graphs(self, top_file_path, bottom_file_path):
        ...
        def create_plot_with_temp(timestamps, pressures, temps, title):
            ...
            # Add temperature curve to main plot's legend
    ...
    def create_station_legend(self, colors, labels, parent_container):
        ...
        legend_frame = QFrame()
        legend_frame.setFrameStyle(QFrame.Shape.Box.value | QFrame.Shadow.Raised.value)
